...
> **交付状态：硬件设计已完成**。本文档对应原理图 `SCH_HPM5E31IPB1_2026-09-13.pdf`、BOM `BOM_HPM5E31IPB1_HPM5E31IPB1_2026-09-13.xlsx` 和引脚分配 `HPM5E31IPB1.csv`。

## 🎯 1. 文档目的

本文档定义 HPM5E31IPB1 核心板的总体硬件组成、功能边界和设计基线，作为原理图、PCB、器件选型和硬件验收的依据。

本文档描述设计要求，不替代器件数据手册、原理图和 PCB 工程文件。涉及具体电压、电流、阻抗、时序和封装参数时，应以最新数据手册和经评审的原理图为准。

## 🧭 2. 设计目标

- 以 HPM5E31IPB1（HPM5E3Y 系列）MCU 为核心，提供电机控制所需的 PWM、ADC、QEI 和调试资源。
...
| 子系统 | 设计内容 | 主要参考资料 |
| --- | --- | --- |
| 主控 | HPM5E31IPB1 MCU、启动配置、复位、时钟和调试连接 | `../Datasheet/HPM5E31IPB1.pdf` |
| 电源 | 输入电源转换、MCU 电源、接口电源和必要的滤波去耦 | `../Datasheet/TLV75733PDBVR.pdf`、`../Datasheet/SY6280AAC.pdf` |
| EtherCAT/以太网 | ESC、PHY、差分收发、复位、参考时钟和管理接口 | `../Pin Assignment/HPM5E31IPB1.csv` |
| CAN | MCAN 信号、收发器、终端匹配和保护 | `../Datasheet/SIT1042AQTK3.pdf` |
| 接口保护 | 外部接口的 ESD/浪涌保护和连接器防护 | `../Datasheet/PRTR5V0U2X.pdf` |
| 调试与扩展 | JTAG、UART、SPI、QEI、PWM 和 ADC 信号 | `../Pin Assignment/HPM5E31IPB1.csv` |
...
最终功能复用必须与原理图、软件引脚初始化和 `HPM5E31IPB1.csv` 三方保持一致。
...
- 原理图和 PCB 交付资料：`Hardware/Schematic/`、`Hardware/BOM/`
- 器件数据手册：`Hardware/Doc/Datasheet/`
- 引脚分配：`Hardware/Doc/Pin Assignment/HPM5E31IPB1.csv`
...
硬件设计交付物已冻结，具体数值以对应版本的原理图、BOM、数据手册和 PCB 制造资料为准。上电、接口、温升、EMC/ESD 等项目仍按《Acceptance Test Criteria.md》执行并记录，不在本规格中预先判定为通过。
